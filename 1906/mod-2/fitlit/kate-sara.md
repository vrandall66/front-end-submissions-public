# FitLit
* Students: Kate Malone, Sara Karsh
...
- Not through iteration 5, but still a pass
...
- Great utilization of bracket notation
- `getBestSleeperByDay()` and `getWorstSleeperByDay()` could be DRYed up and combined
  - same thing with `getHoursForDay()` and `getQualityForDay()`in `Sleep`
...
- test your helper methods whenever you write them
...
- I think your idea about breaking down the group-logic into your `AllUsers` class made sense, but some of the methods you chose to include feel like they belong elsewhere, either in your DOM logic or in a parent class that could be inherited (we'll talk about this next week!)
  - In particular I'm thinking of `getTodayInfo()`
  - I feel like `getUsersWeek()` and `getUsersWeekArray()` could be more appropriate in the `User` or `UserRepository` class
- Would have liked to see you further implement the friends class. This would be a good thing to submit an issue about, so that folks know why the empty class / tests are there
- `findUser` is a great use case for inheritance
...
- We talked about small changes the UI could use, in particular organizing the weekly outputs into a more readable list, and adding in units
